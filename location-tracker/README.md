...
| `LLM_BASE_URL` | `http://host.docker.internal:8080/v1` | OpenAI-compatible LLM endpoint (llama-swap). |
| `LLM_MODEL` | _(auto)_ | Optional override. If unset, resolves the largest non-coder model currently loaded in llama-swap via `stack_shared.llm_model`. |
